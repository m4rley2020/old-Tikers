	...
	if($_REQUEST['user_id'] != '')
	{
		...
		if(mysql_num_rows($get_query_res2)>0)
		{
			...
			while($get_query_date2 = mysql_fetch_array($get_query_res2))
			{
				...
				$follow_total = mysql_num_rows(mysql_query("select id from friend where from_user = '".$user_id."' and status = 2 "));				
				$following_total = mysql_num_rows(mysql_query("select id from friend where to_user = '".$user_id."' and status = 2 "));	
