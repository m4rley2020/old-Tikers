<?php
	...
	if($_REQUEST['storeid'] != '' && $_REQUEST['latitude'] != '' && $_REQUEST['longitude'] != '' && $_REQUEST['start'] != '' && $_REQUEST['start'] >= 0)
	{
		...
		$store_query = mysql_query("select * from store where id='".$store_id."'  ");
		
		if(mysql_num_rows($store_query) > 0 )
		{

			$user_query = "select user.*,( 3959 * acos( cos( radians($latitude) ) * cos( radians(user.latitude ) ) * cos( radians( user.longitude ) - radians($longitude) ) + sin( radians($latitude) ) * sin( radians(user.latitude ) ) ) ) AS range1 from user where user_type = 'Famous' having range1 <= '60'  limit $post_start_limit,10";
