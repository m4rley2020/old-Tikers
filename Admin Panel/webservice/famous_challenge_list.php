<?php
	...
	if($_REQUEST['user_id'] != '' && $_REQUEST['start'] != '' && $_REQUEST['start'] >= 0)
	{
		...
		$get_query2 = "select * from store_challenges where famous_user_id = '".$_REQUEST['user_id']."' and is_deleted = 0 and is_approved = 1 order by id desc limit $post_start_limit,10 ";
		...
		if(mysql_num_rows($get_query_res2)>0)
		{
			...
			{
				$famous_user_id = $get_query_date2['user_id'];
