...
*/
 ...
 $DB=array(
 'db'=>"db530831774",  #default DB, optional
 'user'=>"dbo530831774",#required  
 'pwd'=>"Keshav_777", #required
 'host'=>"db530831774.db.1and1.com",#optional
 'port'=>"3306",#optional
...
function db_connect($nodie=0){
 ...
 if (!$dbh) {
    $err_msg='Cannot connect to the database because: '.mysqli_error($dbh);
...
function do_import(){
 ...
 if ($_FILES['file1'] && $_FILES['file1']['name']){
  ...
  if (!do_multi_sql('', $filename) ){
     $err_msg="Error: ".mysqli_error($dbh);
  }else{
