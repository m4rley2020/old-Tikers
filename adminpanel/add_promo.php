...
{
	$id = $_REQUEST["id"];											
	$fetchquery = "select * from promo_section where id=".$id;
	...
	if(mysqli_num_rows($result) > 0)
	{
		while($row = mysqli_fetch_array($result))
		{
				...
				$promo_image= stripslashes($row['promo_image']);
		}
	}
...
if(isset($_REQUEST['Submit']))
{
	...
	recurse_per_change($_SERVER["DOCUMENT_ROOT"]."/UserFiles");
		...
		$promo_image= addslashes($_REQUEST['promo_image']);
		$promo_image="";

		if ($_FILES["promo_image"]["error"] > 0)
		{
			//echo "Error: " . $_FILES["full"]["error"] . "<br />";
		}
		else
		{
			 $promo_image = rand(1,999).trim($_FILES["promo_image"]["name"]); 
			 move_uploaded_file($_FILES["promo_image"]["tmp_name"],"../promo_image/".$promo_image);
			 
			 auto_change_file_permition("promo_image",$promo_image);
		}	if(isset($_REQUEST['mode']))
		{
			switch($_REQUEST['mode'])
			{
				case 'add' :
					if(GTG_is_dup_add('promo','name',$name))
					{
						unset($_REQUEST['Submit']);	
						location("add_promo.php?mode=add&msg=4");
						return;
					}
				...
				case 'edit' :
					if(GTG_is_dup_edit('promo','name',$name,$_REQUEST['id']))
					{
						unset($_REQUEST['Submit']);	
...
$query = "delete from promo where id=".$_REQUEST['id'];     
	...
	function deletefull1($iid)
	{
		...
		{
			$dfile = $drow['promo_image'];
			if($dfile != "")
			{
				if(file_exists("../promo_image/".$dfile.""))
				{
					unlink("../promo_image/".$dfile."");
				}
			}
		}
            ...
            <div class="container"> <!-- Start : Inner Page container -->
                ...
                <div class="page-header">   <!-- Start : Page Header -->
                    <div class="page-title">
                        <h3><?php echo ($_GET["id"]>0)?"Edit ":"Add "; ?>Promo</h3>
                    ...
                    <div class="col-md-6 col-sm-6">
                        <div class="portlet box blue">
                            <div class="portlet-title">
                                <div class="caption"><i class="fa fa-bars"></i><?php echo ($_GET["id"]>0)?"Edit ":"Add "; ?>Promo</div>
                                
                            </div>
                            <div class="portlet-body">
                                        <form action="add_promo.php" method="post" name="frm" enctype="multipart/form-data" onSubmit="javascript:return keshav_check();" class="form-horizontal row-border" id="validate-1"  novalidate="novalidate">
                                        <input type="hidden" name="mode" id="mode" value="<?= $_REQUEST['mode']; ?>" >
                                         
                    <div class="form-group">
                    <label class="col-md-3 control-label">
                      <span class="required">*</span>Promo Name
                     </label>     
                     <div class="col-md-9">
                     <input class="form-control required" name="name" type="text" id="name" value="<?=$name; ?>" />
                        ...
                        <div class="form-group">
                            <label class="col-md-3 control-label">
                             ...
                             <div class="col-md-9">
                            <input class="form-control" type="file" name="promo_image" id="promo_image" />
                                                            <?php 
                                            if($promo_image!="" && file_exists("../promo_image/".$promo_image))
                                            {
                                                    ?>
                                                    <br />
                                                    <img alt="Image" src="../include/sample.php?nm=../promo_image/<?=$promo_image;?>&mwidth=88&mheight=88" border="0" />
...
function keshav_check()
{
			...
			/* -------------- Image Validation --------------------- */
			if(document.frm.mode.value =="add" && document.frm.promo_image.value.split(" ").join("")=="")										{
