<?php
	...
	if($_REQUEST['user_id'] != '' && $_REQUEST['start'] != '' && $_REQUEST['start'] >= 0)
	{
		...
		if($search_val != "")
		{
			$get_query = "select F.to_user, F.status from friend as F left join user as U on F.to_user = U.id where U.username like '%".$search_val."%' and F.from_user = '".$_REQUEST['user_id']."' and F.status = 2 limit $post_start_limit,10";
		}
		else
		{
