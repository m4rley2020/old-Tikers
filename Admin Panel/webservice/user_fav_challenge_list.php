<?php
	...
	if($_REQUEST['user_id'] != '' && $_REQUEST['start'] != '' && $_REQUEST['start'] >= 0)
	{
		...
		if(mysql_num_rows($get_query_res1)>0)
		{
			...
			{
				$get_query2 = "select * from store_challenges where id = '".$get_query_date1['challange_id']."' order by id desc limit $post_start_limit,10";
				...
				if(mysql_num_rows($get_query_res2)>0)
				{
					while($get_query_date2 = mysql_fetch_array($get_query_res2))
					{
						...
						$socre_rating = number_format(GetValue("store","rating","id",$store_id),2) ;
						if($socre_rating > 0){
							$rating = $socre_rating;
						}
						else{
