<?php
	...
	if($_REQUEST['user_id'] != '' && $_REQUEST['start'] != '' && $_REQUEST['start'] >= 0)
	{
		...
		$user_query = "select U.username,U.user_type, U.profile_image, U.bio, count(F.from_user) as following_counter, social_media_type, social_media_name from user U left join friend F on U.id = F.from_user where U.id = '".$user_id."' and F.status = 2 ";
		...
		if(mysql_num_rows($user_query_res)>0)
		{
			...
			if($user_data['social_media_type'] != null)
			{
				$social_media_type = $user_data['social_media_type'];
			}
			else{
				$social_media_type = "";
			}
			
			if($user_data['social_media_name'] != null){
				$social_media_name = $user_data['social_media_name'];
			}
			else{
				$social_media_name = "";
			}
		...
		$get_query = "select * from post where user_id='".$_REQUEST['user_id']."' limit $post_start_limit,10";
