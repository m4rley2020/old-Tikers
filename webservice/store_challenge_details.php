<?php
	...
	if($_REQUEST['store_id'] != '' && $_REQUEST['challange_id'] != '' )
	{
		...
		$get_query_res2 =   mysqli_query($get_query2)or die(mysqli_error());
		...
		if(mysqli_num_rows($get_query_res2)>0)
		{
			$get_query_date2 = mysqli_fetch_array($get_query_res2);
				...
				$get_store_res =   mysqli_query($get_store_details)or die(mysqli_error());
				
				if(mysqli_num_rows($get_store_res)>0)
				{
					$store_data = mysqli_fetch_array($get_store_res);
